...
export default function Home() {
  ...
  const triggerConfetti = (gumroadUrl) => {
    ...
    setCurrentStep(5);
    // Start a 5-second countdown, then open Gumroad
    setCountdown(5);
    ...
    <div className="modal-card mx-4">
      ...
      <div className="p-6">
        ...
        <div id="modal-step-3" className={`modal-step ${currentStep === 3 ? '' : 'hidden'}`}>
          <h3 className="font-display font-bold text-base mb-4" style={{color: '#0F172A'}}>Preferences &amp; Requirements</h3>
          <div className="space-y-3">
            <div>
            ...
            <div>
              ...
              <textarea id="notes" className="form-input resize-none" rows={2} placeholder="e.g. Grandad has a hip replacement" defaultValue={""} />
            </div>
          </div>
        </div>
        {/* Step 4: Contact Info */}
        ...
        <div id="modal-step-5" className={`modal-step text-center py-6 ${currentStep === 5 ? '' : 'hidden'}`}>
          ...
          <button onClick={() => { setIsIntakeModalOpen(false); setCurrentStep(1); setCountdown(null); }} className="btn-secondary w-full text-center py-3.5 text-sm">I'll Checkout Later</button>
        </div>
      </div>
      {/* Footer nav */}
      {currentStep < 5 && (
