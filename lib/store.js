// In-memory request store for demo & local fallback
...
if (!global._truerouteRequests) {
  global._truerouteRequests = [...defaultRequests];
}

export const localStore = {
  getAll: () => global._truerouteRequests || [],
  get: (id) => {
    const list = global._truerouteRequests || [];
    return list.find(r => r.id === id || r.order_id === id);
  },
  add: (req) => {
    if (!global._truerouteRequests) global._truerouteRequests = [];
    global._truerouteRequests.unshift(req);
  },
  updateStatus: (id, status) => {
    const item = (global._truerouteRequests || []).find(r => r.id === id || r.order_id === id);
