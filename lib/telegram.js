...
export async function sendTelegramNotification(data) {
  const token = process.env.TELEGRAM_BOT_TOKEN || "8606860394:AAH5HrpdXQRu6gyqTJ9n4nJ_7edqKEKHvqw";
  const chatId = process.env.TELEGRAM_CHAT_ID || "1605303737";

  if (!token || !chatId || token.includes("placeholder") || chatId.includes("placeholder")) {
    console.warn("Telegram notification skipped: missing or placeholder token/chatId");
    return;
  }
