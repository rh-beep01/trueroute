...
export async function sendOrderConfirmationEmail(data) {
  ...
  // Sender email: Defaults to custom domain orders@roamify.life or environment override
...
<body style="margin:0; padding:0; background-color:#FAFAF7; font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color:#0F172A;">
  ...
  try {
    ...
    if (response.error && fromEmail !== 'Roamify <onboarding@resend.dev>') {
      console.warn('Resend custom domain not fully verified yet. Retrying with onboarding fallback...', response.error);
    ...
    console.log('Order confirmation email sent successfully to', clientEmail, 'Email ID:', response.data?.id);
