...
export async function sendOrderConfirmationEmail(data) {
  ...
  const mustSee = escapeHtml(data.must_see || '');
  ...
  const placeTypes = escapeHtml(data.place_types || '');
  const avoidPlaces = escapeHtml(data.avoid_places || '');
            ...
            <td style="padding:0 36px 24px 36px;">
              <div style="background:#F8FAFC; border:1px solid #E2E8F0; border-radius:12px; padding:24px; text-align:center;">
                <span style="display:inline-block; background:#E8F2EC; color:#2E6F40; font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:1px; padding:4px 10px; border-radius:20px; margin-bottom:12px;">
                  ✓ Questionnaire Details Received
                </span>
                <h2 style="margin:0 0 8px 0; font-size:20px; font-weight:700; color:#0F172A;">
                  We've got your trip details, ${clientName}!
                </h2>
                <p style="margin:0 0 16px 0; font-size:14px; line-height:1.6; color:#475569;">
                  Thank you for submitting your custom family travel preferences for <strong>${destPrimary}</strong>. Our human planners are reviewing every age, mobility requirement, and pacing preference.
                </p>
                <div style="display:inline-block; background:#FFFFFF; border:1px solid #CBD5E1; border-radius:8px; padding:8px 16px;">
                  <span style="font-size:12px; color:#64748B;">Tracking Order ID:</span>
                  <strong style="font-size:14px; color:#0F172A; font-family:monospace; margin-left:6px;">${orderId}</strong>
                </div>
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:0 36px 24px 36px;">
              ...
              <table width="100%" cellspacing="0" cellpadding="8" style="font-size:13px; border-collapse:collapse; background:#FFFFFF; border:1px solid #F1F5F9; border-radius:8px;">
                ...
                ${mustSee ? `
                <tr style="border-bottom:1px solid #F1F5F9;">
                  <td style="color:#64748B; font-weight:600;">Must-See Sights:</td>
                  <td style="color:#0F172A; font-weight:600;">${mustSee}</td>
                </tr>` : ''}
                <tr style="border-bottom:1px solid #F1F5F9;">
                ...
                ${placeTypes ? `
                <tr style="border-bottom:1px solid #F1F5F9;">
                  <td style="color:#64748B; font-weight:600;">Places to Roam:</td>
                  <td style="color:#0F172A;">${placeTypes}</td>
                </tr>` : ''}
                <tr style="border-bottom:1px solid #F1F5F9;">
                ...
                ${avoidPlaces ? `
                <tr style="border-bottom:1px solid #F1F5F9;">
                  <td style="color:#64748B; font-weight:600;">Places to Avoid:</td>
                  <td style="color:#0F172A;">${avoidPlaces}</td>
                </tr>` : ''}
                ${notes !== 'None' ? `
                <tr>
